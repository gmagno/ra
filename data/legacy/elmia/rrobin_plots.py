...
pkl_fname_res = 'elmia_odeon_400k'        # simulation results name
...
def plot_vs_freq(sou, participant, js, jrec, softname, par, y_limits):
    ...
    #     linewidth = 3)
    plt.plot(sou[js].freq[1:-1], trem_parameter[1:-1], 's-r', label = softname,
        linewidth = 3)
...
def plot_vs_srpairs(sou, participant, jf, softname, par, y_limits):
    ...
    for js in np.arange(2):
        for jrec in np.arange(6):
            ...
            trem_parameter.append(trem_parameter_vec[jf+1])
            meas_parameter_vec = getattr(participant[14].sou[js].rec[jrec], par)
    ...
    x_string = ['S1R1','S1R2','S1R3', 'S1R4','S1R5','S1R6', 'S2R1','S2R2','S2R3', 'S2R4','S2R5','S2R6']
    ...
    plt.title('Frequency band: ' + str(sou[0].freq[jf+1]) + ' Hz')
    plt.grid(linestyle = '--')
    plt.legend(loc = 'best')
...
def plot_main_error(sou, participant, softname, par):
    ...
    for jpart, p in enumerate(par):
        ...
        for js in np.arange(2):
            for jrec in np.arange(6):
                meas_parameter.append(getattr(participant[14].sou[js].rec[jrec], p))
                trem_vec = getattr(sou[js].rec[jrec], p)
                trem_parameter.append(trem_vec[1:-1])
        # The error of our simulation
        if p != 'T30' and p != 'EDT':
            error_trem = np.mean(np.abs(np.array(trem_parameter) - np.array(meas_parameter))) / jnd[p]
            print('other par if: {}'.format(p))
        else:
            print('t30 else: {}'.format(p))
            error_dec = np.zeros(2*6)
            for js in np.arange(2*6):
