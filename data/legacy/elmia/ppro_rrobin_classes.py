...
class RecRoundRobin():
    def __init__(self, file_name, participant, jsr):
        ...
        T30_i = 2 # valid for the round robin 3
        step = 1 # valid for the round robin 3
        ...
        for jf, f in enumerate(freq):
            ...
            self.EDT[jf] = sheet.cell_value(idf+9, jsr)
            self.D50[jf] = sheet.cell_value(idf+2*9, jsr)
            self.C80[jf] = sheet.cell_value(idf+3*9, jsr)
            self.Ts[jf] = sheet.cell_value(idf+4*9, jsr)
            self.G[jf] = sheet.cell_value(idf+5*9, jsr)
            self.LF[jf] = sheet.cell_value(idf+6*9, jsr)
            self.LFC[jf] = sheet.cell_value(idf+7*9, jsr)
            idf += step